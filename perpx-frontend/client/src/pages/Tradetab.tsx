...
export default function Trade() {
  ...
  // ⑤ Event listener（自分のアドレスだけ処理）
  const onMarginDeposited = (
    user: string,
    amount: bigint,
    timestamp: bigint
  ) => {
  ...
  // ⑥ 登録
  router.on("MarginDeposited", onMarginDeposited);

  // ⑦ クリーンアップ
  return () => {
    router.off("MarginDeposited", onMarginDeposited);
  };

}, [address]); // 👈 外部から変わるのは address だけ
...
useEffect(() => {
  ...
  <div className="text-sm text-white">
    Transaction history content here
            ...
            {/* ================================
               POSITIONS TAB
               Event: PositionOpened / PartiallyClosed
            ================================ */}

            ③ ここに「Positions」を表示する（Event）
